...
class ML1MBatchKGEnvironment(object):
    def __init__(self, args, dataset_str, max_acts, max_path_len=3, state_history=1):
        super(ML1MBatchKGEnvironment, self).__init__()
        self.args = args
        self.max_acts = max_acts
        self.act_dim = max_acts + 1  # Add self-loop action, whose act_idx is always 0.
        self.max_num_nodes = max_path_len + 1  # max number of hops (= #nodes - 1)
    ...
    def _get_actions(self, path, done):
        ...
        for r, next_node_id in candidate_acts:
            ...
            elif next_node_type == CINEMATOGRAPHER:
                src_embed = user_embed + self.embeds[CINEMATOGRAPHY_BY][0]
            elif next_node_type == PRODUCTION_COMPANY:
                src_embed = user_embed + self.embeds[PRODUCED_BY_COMPANY][0]
            elif next_node_type == COMPOSER:
                src_embed = user_embed + self.embeds[COMPOSED_BY][0]
            elif next_node_type == ACTOR :
                src_embed = user_embed + self.embeds[STARRING][0]
            elif next_node_type == COUNTRY:
                src_embed = user_embed + self.embeds[PRODUCED_IN][0]
            elif next_node_type == WIKIPAGE: 
                src_embed = user_embed + self.embeds[MENTION][0]
            elif next_node_type == EDITOR :
                src_embed = user_embed + self.embeds[EDITED_BY][0]
            elif next_node_type == WRITTER:
                src_embed = user_embed + self.embeds[WROTE_BY][0]
            elif next_node_type == DIRECTOR:  
                src_embed = user_embed + self.embeds[DIRECTED_BY][0]
            else:  # WIKIPAGE, CATEGORY, RELATED_PRODUCT
                src_embed = user_embed + self.embeds[WATCHED][0] + self.embeds[r][0]
            score = np.matmul(src_embed, self.embeds[next_node_type][next_node_id])
            # This trimming may filter out target products!
            # Manually set the score of target products a very large number.
            # if next_node_type == PRODUCT and next_node_id in self._target_pids:
    ...
    def _kg_get_reward(self, path):
        ...
        if curr_node_type == PRODUCT:
            ...
            if curr_node_id in self.kg(USER, uid)[WATCHED]: score += 1
            else: score += 0.0
